...
module.exports = (db) => {
      ...
      db.bill.createNewBillInGroup(
        ...
        (error, result) => {
          if (result) {
            ...
            db.bill.createUserBillLink(
              bill_id,
              group_id,
              bill_information,
              (error, result2) => {
                if (result2) {
                  db.bill.updateNetTable(
                    bill_id,
                    group_id,
                    split_array,
                    payer_id,
  ...
  let settleGroupBillControllerCallback = (request, response) => {
    ...
    let user_name = request.cookies['user_name'];
    if (cookieAvailable) {
      db.bill.settleNetTableForUserByGroup(
        user_id,
        group_id,
        settler_id,
        (error, result) => {
          if (result) {
            db.bill.settleSplitAmountByGroup(
              user_id,
              group_id,
              settler_id,
              (error, result) => {
                if (result) {
                  db.main.updateActivityForSettleByGroup(
                    user_id,
                    user_name,
                    settler_id,
                    'settled',
                    ...
                    (error, result) => {
                      if (result) {
                        ...
                        response.send(
                          'UNABLE TO UPDATE ACTIVITY FOR SETTLING DEBT BY GROUP'
                        );
                      }
                    }
                  );
          ...
          if (result) {
            ...
            async function update1by1() {
              for (let i = 0; i < billDetails.split_amount.length; i++) {
                let waiter = await db.bill.updateSplitAmount(
                  billDetails.user_id[i],
                  bill_id,
                  billDetails.split_amount[i],
                  (error, result) => {
                    if (result) {
            ...
            async function updateNetTable() {
              for (let i = 0; i < billDetails.split_amount.length; i++) {
                if (parseInt(payer_id) !== parseInt(billDetails.user_id[i])) {
                  let waiter2 = await db.bill.updateNetTableForEdit(
                    billDetails.user_id[i],
                    billDetails.split_amount[i],
                    payer_id,
                    net_table_id[count].id,
                    (error, result) => {
                      if (result) {
                  ...
                  let waiter3 = await db.bill.updateNetTableForEdit(
                    payer_id,
                    negativeNet,
                    billDetails.user_id[i],
                    net_table_id[count].id,
            ...
            db.main.updateActivityForEdit(
              ...
              (error, result) => {
                if (result) {
                  setTimeout(function () {
                    response.redirect(
                      '/blitt/groupList/' + group_id + '/' + bill_id
                    );
                  }, 2000);
                } else {
            ...
            response.send('CANOT GET NET_tABLE');
          }
        });
      } else {
  ...
  let deleteBillControllerCallback = (request, response) => {
    ...
    db.bill.deleteSingleBill(bill_id, (error, result) => {
      if (result) {
        ...
        db.main.updateActivityForDelete(
          user_id,
          user_name,
          'deleted',
          group_id,
